class ShippingAgentConverter {
    constructor() {
        this.agents = {
            ...
            'niuniubox.com': {
                ...
                build: (itemId, platform) => `https://www.niuniubox.com/product-detail/?platform=${platform}&id=${itemId}`
            },
            ...
            'wemimi.com': {
                ...
                build: (originalUrl) => `https://www.wemimi.com/#/home/productDetail?productLink=${encodeURIComponent(originalUrl)}`
            },
            ...
            'mambabuy.com': {
                ...
                build: (encryptedKeyword) => `https://mambabuy.com/goods/detail?keyword=${encryptedKeyword}`
            },
        ...
        this.platformMappings = {
            ...
            'micro': 'weidian.com',
            '1688': '1688.com',
            '2': 'weidian.com' // For usfans platform mapping
        };
    }
    ...
    parseAgentUrl(url, agentDomain, config) {
        switch (config.type) {
            ...
            case 'encrypted':
                const encryptedMatch = url.match(config.patterns.extract);
                if (encryptedMatch) {
                    // For encrypted keywords, we can't determine the original URL easily
                    // but we can still track the agent and encrypted identifier
                    return {
                        type: 'agent',
                        agent: agentDomain,
                        itemId: encryptedMatch[1],
                        platform: 'unknown',
                        originalUrl: null // Can't determine original URL from encrypted data
                    };
                }
                break;

            case 'special':
                const specialMatch = url.match(config.patterns.extract);
                if (specialMatch) {
                    return {
                        type: 'agent',
                        agent: agentDomain,
                        itemId: specialMatch[1],
                        platform: '1688.com', // yoybuy typically handles 1688
                        originalUrl: this.buildOriginalUrl('1688.com', specialMatch[1])
                    };
                }
                break;
        }
    ...
    convertUrl(sourceUrl, targetAgent) {
        ...
        // Special handling for encrypted agents
        if (agentConfig.type === 'encrypted') {
            // If we can't determine the original URL, we can't convert to encrypted format
            if (!parsed.originalUrl) return null;
            // For now, we'll just return null for encrypted agents as we can't generate the encrypted keyword
            return null;
        }
    ...
    getPlatformCode(platform, agent) {
        ...
        const mappings = {
            'taobao.com': { default: 'taobao', 'acbuy.com': 'TB', 'usfans.com': '1' },
            'weidian.com': { default: 'weidian', 'npbuy.com': 'WD', 'cnfans.com': 'WEIDIAN', 'usfans.com': '2', 'niuniubox.com': 'micro' },
            '1688.com': { default: '1688', 'usfans.com': '3' },
            'tmall.com': { default: 'tmall', 'usfans.com': '4' }
        };
